...
# Bounds for the three dentaku operations whose cost grows with the magnitude of an operand.
...
# `{{a}} ^ {{b}}` or `ROUND({{a}}) << {{b}}` could otherwise burn CPU or memory for minutes. The checks are
# called from the operators themselves (Ccn::BoundedExponentiation, Ccn::BoundedShift, prepended in
# config/initializers/zz_ccn_dentaku.rb), so they run exactly where dentaku evaluates them: an untaken IF
# branch is never checked and a nested `^` is bounded from the inside out. Submitters::SubmitValues turns
# the errors into HTTP 422 messages.
module Ccn
